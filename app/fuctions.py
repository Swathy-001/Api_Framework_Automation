...
async def create_user(user: User):
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 201:  # 201 is for successful resource creation
...
async def update_user(user_id: int, user: User):
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:  # 200 means success in update
...
async def delete_user(user_id: int):
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 204:  # 204 is for successful deletion (no content)
